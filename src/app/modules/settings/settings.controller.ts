...
const upsertDisclaimer = catchAsync(async (req: Request, res: Response) => {
  const { description } = req.body;

  const result = await SettingsService.upsertDisclaimer(description);
...
const getDisclaimer = catchAsync(async (req: Request, res: Response) => {
  const result = await SettingsService.getDisclaimer();
